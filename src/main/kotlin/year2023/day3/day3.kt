...
data class Neighbor(val char: Char, val row: Int, val col: Int) {
    ...
    override fun equals(other: Any?): Boolean {
        ...
        val casted = other as Neighbor

        return char == casted.char && row == casted.row && col == casted.col
    }
...
class Num(val row: Int, val col: Int) {
    private val neighbors = hashSetOf<Neighbor>()
    private var tempNum = ""
    private var value = 0L

    fun addNeighbor(char: Char, row: Int, col: Int) {
        if (char == '.' || char.isDigit()) {
            return
        }

        neighbors.add(Neighbor(char, row, col))
    }
    ...
    fun addDigit(char: Char) {
        tempNum += char
        value = tempNum.toLong()
    }

    fun hasSymbolNeighbor(): Boolean {
        return neighbors.size > 0
    }

    fun getNeighbors(): HashSet<Neighbor> {
        return neighbors
    }
}

fun part1(input: String): MutableList<Num> {
    val lines = input.lines()

    var tempNumber = Num(0, 0)
    val allNumbers = mutableListOf<Num>()

    for ((row, line) in lines.withIndex()) {
        var i = 0
        if (tempNumber.getValue() > 0) {
            allNumbers.add(tempNumber)
            tempNumber = Num(row, 0)
        }

        do {
            val char = line[i]
            if (char.isDigit()) {
                tempNumber.addDigit(char)

                //check adjacent squares and add the neighbors
                if (i + 1 < line.length) {
                    tempNumber.addNeighbor(lines[row][i + 1], row, i + 1)
                }
                if (i > 0) {
                    tempNumber.addNeighbor(lines[row][i - 1], row, i - 1)
                }
                if (row > 0) {
                    tempNumber.addNeighbor(lines[row - 1][i], row - 1, i)
                    if (i + 1 < line.length) {
                        tempNumber.addNeighbor(lines[row - 1][i + 1], row - 1, i + 1)
                    }
                    if (i > 0) {
                        tempNumber.addNeighbor(lines[row - 1][i - 1], row -1, i - 1)
                    }
...
fun part2(input: String) {
    val allNums = part1(input)
    val allGears = allNums.filter { it.getNeighbors().any { n -> n.char == '*' } }

    val gearMap = mutableMapOf<Neighbor, MutableList<Long>>()

    for (num in allGears) {
        println("${num.getValue()} starts at ${num.row}, ${num.col} ${num.getNeighbors()}")
        val neighbor = num.getNeighbors().single { it.char == '*' }
        val list = gearMap.getOrDefault(neighbor, mutableListOf())
        list.add(num.getValue())

        gearMap.putIfAbsent(neighbor, list)
    }

    val values = gearMap.values
        .filter { it.size > 1 }
        .sumOf { it.fold(1L) { acc, l -> acc * l} }
    println(values)
}

fun main(args: Array<String>) {
    val part1 = part1(puzzleInput)

    val withNeighbors = part1.filter { it.hasSymbolNeighbor() }
//    for (node in withNeighbors.sortedBy { it.getValue() }) {
//        println("${node.getValue()}")
//    }

    val sum = withNeighbors.sumOf { it.getValue() }

    println("part1 $sum")

    val part2 = part2(puzzleInput)
