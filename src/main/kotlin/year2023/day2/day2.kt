...
data class Cube(val number: Int, val color: String)

class Game(val gameId: Int) {
    val cubesShown = mutableListOf<List<Cube>>()

    fun addCubes(cubes: List<Cube>) {
        cubesShown.add(cubes)
    }

    fun getCubePower(): Int {
        var redCubes = 0
        var greenCubes = 0
        var blueCubes = 0

        for (round in cubesShown) {
            for (cube in round) {
                if (cube.color == "green") {
                    if (cube.number > greenCubes) greenCubes = cube.number
                }
                if (cube.color == "blue") {
                    if (cube.number > blueCubes) blueCubes = cube.number
                }
                if (cube.color == "red") {
                    if (cube.number > redCubes) redCubes = cube.number
                }
            }
        }

        return greenCubes * redCubes * blueCubes
    }

    fun isPossible(redCubes: Int = 12,
                   greenCubes: Int = 13,
                   blueCubes: Int = 14): Boolean {
        var possible = true

        for (cubeShow in cubesShown) {
            for (cube in cubeShow) {
                if (cube.color == "green") {
//                    println("checking ${cube.number} green cubes shown against ${greenCubes}")
                    if (cube.number > greenCubes) possible = false
                }
                if (cube.color == "blue") {
//                    println("checking ${cube.number} green cubes shown against ${blueCubes}")
                    if (cube.number > blueCubes) possible = false
                }
                if (cube.color == "red") {
//                    println("checking ${cube.number} green cubes shown against ${redCubes}")
                    if (cube.number > redCubes) possible = false
                }
            }
        }
...
fun main(args: Array<String>) {
    ...
    for (line in puzzleInput.lines()) {
        ...
        val gameId = lineSplit[0].removePrefix("Game ").toInt()

        val shown = lineSplit[1].split(";")
        val roundsShown = mutableListOf<Cube>()
        val game = Game(gameId)

        for (round in shown) {
            val cubesShown = round.split(",")
            for (badName in cubesShown) {
                val parts = badName.trim().split(" ")
                val c = Cube(parts[0].toInt(), parts[1])
                roundsShown.add(c)
            }
        }
        game.addCubes(roundsShown)
    ...
    for (game in gameList) {
        println("Game ${game.gameId} is possible? ${game.isPossible()}, power ${game.getCubePower()}")
        if (game.isPossible()) {
            result += game.gameId
        }
        totalPower += game.getCubePower()
    }
    println("result is $result")
    println("total power is $totalPower")
}
