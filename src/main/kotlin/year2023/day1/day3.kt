...
fun part1(input: String) {
    ...
    for (line in input.lines()) {
        ...
        val finalValue = "${line.first { it.isDigit() }}${line.last { it.isDigit()}}"

        if (finalValue.isNotEmpty()) {
            results.add(finalValue.toInt())
        }

    }
    println("Sum of all lines: ${results.sum()}")
}

fun part2(input: String) {
    val newInput = StringBuilder()
    for (line in input.lines()) {
        // I totally stole this solution because I had to understand how it worked
        var newLineInput = line.replace("one", "one1one")
        newLineInput = newLineInput.replace("two", "two2two")
        newLineInput = newLineInput.replace("three", "three3three")
        newLineInput = newLineInput.replace("four", "four4four")
        newLineInput = newLineInput.replace("five", "five5five")
        newLineInput = newLineInput.replace("six", "six6six")
        newLineInput = newLineInput.replace("seven", "seven7seven")
        newLineInput = newLineInput.replace("eight", "eight8eight")
        newLineInput = newLineInput.replace("nine", "nine9nine")
        newLineInput = newLineInput.replace("zero", "zero0zero")
        newInput.appendLine(newLineInput)
    }

    part1(newInput.toString())
}
